...
def update_model_db(db_path):
    ...
    try:
        ...
        print("You are not connected to the internet! check your connectivity and try again.")
...
def get_model(model_repo, model_path):
    ...
    if not models_repo.exists():
        raise Exception("model database is not available! run 'nobrainer-zoo init' first!")
